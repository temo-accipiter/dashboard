...
describe('GitHub Mock Data', () => {
  ...
  describe('getMockRepositoryPullRequests', () => {
    ...
    it('should filter by state', () => {
      const prs = getMockRepositoryPullRequests('acme', 'dashboard', {
        state: 'open',
      })
    ...
    it('should paginate results', () => {
      const page1 = getMockRepositoryPullRequests('facebook', 'react', {
        perPage: 1,
        page: 1,
      })
      const page2 = getMockRepositoryPullRequests('facebook', 'react', {
        perPage: 1,
        page: 2,
      })
    ...
    it('should have different PRs for different repositories', () => {
      ...
      if (repo1PRs.length > 0 && repo2PRs.length > 0) {
        expect(repo1PRs[0]!.repository.full_name).not.toBe(
          repo2PRs[0]!.repository.full_name
        )
      }
    })
  })
