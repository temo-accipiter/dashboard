  ...
  describe('GitHubAdapterImpl', () => {
    ...
    describe('getRepositoryPullRequests', () => {
      it('should return pull requests for a specific repository', async () => {
        const adapter = new GitHubAdapterImpl()
        const prs = await adapter.getRepositoryPullRequests(
          'octocat',
          'hello-world'
        )
        ...
        if (prs.length > 0) {
          expect(
            prs.every((pr) => pr.repository.full_name === 'octocat/hello-world')
          ).toBe(true)
        }
      })
      ...
      it('should filter repository PRs by state', async () => {
        ...
        if (prs.length > 0) {
          expect(prs.every((pr) => pr.state === 'open')).toBe(true)
          expect(
            prs.every((pr) => pr.repository.full_name === 'acme/dashboard')
          ).toBe(true)
        }
      })
      ...
      it('should handle case-insensitive repository names', async () => {
        const adapter = new GitHubAdapterImpl()
        const prs1 = await adapter.getRepositoryPullRequests(
          'OCTOCAT',
          'HELLO-WORLD'
        )
        const prs2 = await adapter.getRepositoryPullRequests(
          'octocat',
          'hello-world'
        )
      ...
      it('should support pagination for repository PRs', async () => {
        const adapter = new GitHubAdapterImpl()
        const page1 = await adapter.getRepositoryPullRequests(
          'facebook',
          'react',
          { perPage: 1, page: 1 }
        )
        const page2 = await adapter.getRepositoryPullRequests(
          'facebook',
          'react',
          { perPage: 1, page: 2 }
        )
